		...
		: ThreadedKernel(argc, argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        // Get execfile & its priority & burst time from argv, then save them.
            ...
            cout << "Creating thread for executable: " << execfile[execfileNum] << " with priority " << priority << " and burst time " << burstTime << endl;
	    }
	    //<TODO>
...
ForkExecute(Thread *t)
{
    ...
    DEBUG(dbgThread, "Entering ForkExecute for Thread ID " << t->getID() << " at system time " << kernel->stats->totalTicks);
...
UserProgKernel::InitializeOneThread(char* name, int priority, int burst_time)
{
    ...
    cout << "Initializing thread: " << name << endl;
    // Create new thread object
    t[threadNum] = new Thread(name,threadNum);

    // Assign priority and burst time
    t[threadNum]->setPriority(priority);
    t[threadNum]->setRemainingBurstTime(burst_time);


    // Set up the address space for the executable file
    t[threadNum]->space = new AddrSpace();

    t[threadNum]->space->Load(name);  // Assumes LoadProgram is a method that loads the program

    // Fork the thread to start execution
    t[threadNum]->Fork((VoidFunctionPtr) &ForkExecute, (void *)t[threadNum]);
    cout << "Thread " << name << " initialized and forked." << endl;
